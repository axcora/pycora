...
class SimpleSSG:
    ...
    def generate_pages(self):
        ...
        for md_file in pages_dir.glob('*.md'):
            ...
            self.total_files += 1
    def generate_donate(self):
         """Generate donate page"""
         html = self.env.get_template('donate.html').render(
             site=self.config.get('site', {})
         )
         Path('output/donate/index.html').parent.mkdir(parents=True, exist_ok=True)
         Path('output/donate/index.html').write_text(html, encoding='utf-8')
         self.total_files += 1
         print(f"{Colors.GREEN}💝 Donate page generated{Colors.RESET}")
